...
impl<T> Proof<T> {
    ...
    pub fn new(algorithm: &'static Algorithm, root_hash: Vec<u8>, lemma: Lemma, value: T, height: isize) -> Self {
        Proof {
            algorithm,
            root_hash,
            lemma,
    ...
    pub fn validate(&self, root_hash: &[u8]) -> bool {
        ...
        self.lemma.validate(self.height - 1, self.algorithm)
    }
    ...
    pub fn get_sibling_vec(&self) -> Vec<Option<Vec<u8>>> {
        let mut res = vec![];
        self.lemma.recursive_lemma_sibling(&mut res);
        res.reverse();
        res
    }
...
impl Lemma {
    ...
    fn new_tree_proof<T>(
        ...
        Lemma::new(left, needle)//内部完成 recursive ,search the vaild path
            .map(|lemma| {
    ...
    fn validate(&self, height: isize, algorithm: &'static Algorithm) -> bool {
        match self.sub_lemma {
            ...
            Some(ref sub) => match self.sibling_hash {
                ...
                Some(Positioned::Left(ref hash)) => {
                    let combined = algorithm.hash_nodes(height, hash, &sub.node_hash);
                    let hashes_match = combined.as_ref() == self.node_hash.as_slice();
                    hashes_match && sub.validate(height - 1, algorithm)
                }

                Some(Positioned::Right(ref hash)) => {
                    let combined = algorithm.hash_nodes(height,&sub.node_hash, hash);
                    let hashes_match = combined.as_ref() == self.node_hash.as_slice();
                    hashes_match && sub.validate(height - 1, algorithm)
                }
            },
    ...
    fn recursive_lemma_sibling(&self,res :&mut Vec<Option<Vec<u8>>>){
        match self.sub_lemma {
            None => (),
            Some(ref sub) => match self.sibling_hash {
                None => (),
                Some(Positioned::Left(ref hash)) => {
                    res.push(Some(hash.clone()));//clone() equals to to_vec()
                    sub.recursive_lemma_sibling(res);
                },
                Some(Positioned::Right(ref hash)) => {
                    res.push(Some(hash.clone()));
                    sub.recursive_lemma_sibling(res);
                }
            }
        }
